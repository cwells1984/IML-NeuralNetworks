...
if __name__ == '__main__':
    ...
    print("AUTOENCODE - WEIGHT UPDATES")
    df_house = dataprep.prepare_house_votes_84('datasets/house-votes-84.data')
    df_house = preprocessing.encode_onehot(df_house, "party")
    df_house = df_house.iloc[0:1]
    ae = AutoEncodedNetwork(encode_learn_rate=0.1, learn_rate=0.01, num_encoding=15, num_hidden1=6, type='Classifier', min_loss=0.5)
    ae.fit(df_house, ['party_0', 'party_1'], verbose=True)

    # AUTOENCODE
    print("AUTOENCODE - RUN")
    df_house = dataprep.prepare_house_votes_84('datasets/house-votes-84.data')
    df_house_part = preprocessing.df_stratified_partition(df_house, 'party', 5)
    ...
    ae = AutoEncodedNetwork(encode_learn_rate=0.1, learn_rate=0.1, num_encoding=15, num_hidden1=6, type='Classifier', min_loss=1, training_cutoff=1)
